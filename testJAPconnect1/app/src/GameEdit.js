...
class GameEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.match.params.id !== 'new') {
      const game = await (await fetch(`/api/game/${this.props.match.params.id}`)).json();
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormGroup>
            ...
            <Input type="text" name="cost" id="cost" value={item.cost || ''}
                   onChange={this.handleChange} autoComplete="cost-level1"/>
          </FormGroup>
          <FormGroup>
            <Label for="category">Category</Label>
            <Input type="text" name="category" id="category" value={item.category || ''}
                   onChange={this.handleChange} autoComplete="category-level1"/>
          </FormGroup>
          <FormGroup>
